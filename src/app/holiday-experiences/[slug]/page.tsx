...
export default async function ExperienceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    ...
    <section className="bg-[var(--warm-ivory)] px-4 py-16 sm:px-6 lg:px-8"><div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[1.5fr_1fr]"><div><h2 className="font-playfair mb-5 text-3xl font-bold text-[var(--forest-canopy)]">Overview</h2><p className="text-lg leading-8 text-stone-700">{experience.description}</p>{experience.highlights.map((h: string, i: number) => <div key={i} className="mt-10"><h3 className="font-playfair mb-3 text-2xl font-bold text-[var(--forest-canopy)]">{h}</h3><p className="leading-7 text-stone-700">{h}</p></div>)}</div><div className="relative h-80 overflow-hidden rounded-2xl lg:h-[28rem]"><Image src={experience.imageUrl || "/images/cats-logo.jpg"} alt={`${experience.title} in East Africa`} fill className="object-cover" /></div></div></section>
